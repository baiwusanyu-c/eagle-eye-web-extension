...
function buildEnvVar() {
    ...
    // 根据命令参数修改要导出的环境变量
    let replaceTargetInfo = readFileInfo.split(' = ')[0]
    readFileInfo = `${replaceTargetInfo} = '${env}'`
    // 生成文件覆盖
    exists(filePath,  function(isExist:boolean) {
        if(!isExist){
            mkdir(filePath,()=>{
                writeFile('.env',readFileInfo, (err:Error)=>{
                    if(err) {
                        console.log(err);
                        return
                    }
            ...
            writeFile(`${filePath}/.env`,readFileInfo,(err:Error)=>{
                if(err) {
                    console.log(err);
                    return
                }
